...
//to edit project
export const projectsPut = (id, pData) => dispatch => {
    dispatch(projectPutting(true));

    const pDataKeys = Object.keys(pData);
    fetch(baseURL + 'projects/' + id.toString())
        .then(response => {
            if(response.ok){
                return response;
            }
        ...
        .then(project => {
            ...
            return fetch(baseURL + 'projects/' + id.toString(), {
                method: 'PUT',
...
//to edit employee
export const employeesPut = (id, eData) => dispatch => {
    dispatch(employeePutting(true));

    fetch(baseURL + 'employees/' + id.toString())
        .then(response => {
            if(response.ok){
                return response;
            }
        ...
        .then(employee => {
            ...
            return fetch(baseURL + 'employees/' + id.toString(), {
                method: 'PUT',
                body: JSON.stringify(eData),
                headers: {
                    'Content-Type' : 'application/json'
    ...
    fetch(baseURL + 'employees')
        ...
        .then(employees => {
            ...
            async function putBackEmployees(){
                for(let i=0; i < presenteeList.length; i++){
                    await fetch(baseURL + 'employees/' + employees[presenteeList[i] - 1]._id.toString(), {
                        method: 'PUT',
